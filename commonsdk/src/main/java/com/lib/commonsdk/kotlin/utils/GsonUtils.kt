...
fun toJson(any: Any): String {
    return toJson(GsonUtils.gson, any)
}
...
fun toJson(src: Any, typeOfSrc: Type): String {
    return toJson(GsonUtils.gson, src, typeOfSrc)
}
...
fun <T> fromJson(json: String, type: Class<T>): T {
    return fromJson(GsonUtils.gson, json, type)
}
...
fun <T> fromJson(reader: Reader, type: Class<T>): T {
    return fromJson(GsonUtils.gson, reader, type)
}
...
object GsonUtils {
    ...
    private val gsonMap by lazy {
        mutableMapOf<String, Gson>()
    }
    ...
    val gson: Gson
        get() {
            val gsonDelegate = gsonMap[KEY_DELEGATE]
            if (gsonDelegate != null) {
                return gsonDelegate
            }
            var gsonDefault = gsonMap[KEY_DEFAULT]
            if (gsonDefault == null) {
                gsonDefault = createGson()
                gsonMap[KEY_DEFAULT] = gsonDefault
            }
            return gsonDefault
        }
