 ...
 */
public class LineChartCustomView extends View {
    ...
    public LineChartCustomView(Context context) {
        super(context);
        initDefault(context);
    }

    public LineChartCustomView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initDefault(context);
        setAttr(context, attrs);
    }

    public LineChartCustomView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initDefault(context);
        setAttr(context, attrs);
    }
     ...
     *
     * @see LineChartCustomView#setMainValuesStyle
     */
    ...
    private void limitsCorrection() {
        ...
        for (Line line : mLines)
            for (LinePoint point : line.getPoints()) {
                ...
                mMinX = Math.min(point.getX(), mMinX);
                mMinY = Math.min(point.getY(), mMinY);
            }

        mMaxX = Math.max(mMaxX, mViewPortRight);
        mMaxY = Math.max(mMaxY, mViewPortTop);
        mMinX = Math.min(mMinX, mViewPortLeft);
        mMinY = Math.min(mMinY, mViewPortBottom);
    }
