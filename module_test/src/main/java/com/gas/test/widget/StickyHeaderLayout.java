 ...
 * 并且使用{@link RecyclerView.Adapter},就可以实现列表头部吸顶功能。
...
//                    (RecyclerView.ViewHolder) (view.getTag(VIEW_TAG_HOLDER)));
...
//     * @return 返回偏移量。
//     */
//    private float calculateOffset(RecyclerView.Adapter gAdapter, int firstVisibleItem, int groupPosition) {
////        int groupHeaderPosition = gAdapter.getPositionForGroupHeader(groupPosition);
////        if (groupHeaderPosition != -1) {
